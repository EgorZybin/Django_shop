...
class OrderView(APIView):
    ...
    def get(self, request: HttpRequest) -> Response:
        ...
        ).prefetch_related("products").select_related("profile")  # Получаем заказы пользователя
        serialize = OrderSerializer(data, many=True)  # Сериализуем данные
        return Response(serialize.data, status=status.HTTP_200_OK)

    def post(self, request: HttpRequest) -> Response:
        cart = Basket(request)  # Инициализируем корзину
        ...
        ).defer("description")  # Получаем объекты продуктов
...
class OrderDetailView(APIView):
    ...
    def get(self, request: HttpRequest, id: int) -> Response:
        ...
        cart = Basket(request)  # Получаем сессию корзины
        ...
        try:
            ...
            query = CountProductinOrder.objects.filter(
                order_id=id
            ).prefetch_related("products")  # Получаем объект заказа
            prods = {
    ...
    def post(self, request: HttpRequest, id: int) -> Response:
        cart = Basket(request)  # Инициализируем корзину
        order = get_object_or_404(Order, pk=id)  # Получаем добъект заказа
        data = request.data
...
class PaymentView(APIView):
    ...
    def post(self, request: HttpRequest, id: int) -> Response:
        order = get_object_or_404(Order.objects.defer("products"), id=id)  # Обработка отсутствующего заказа
        order.status = "Оплачен"  # Изменяем статус заказа
        order.save()  # Сохраняем данные в db

        return Response({"status": "Order paid successfully"}, status=status.HTTP_200_OK)  # Уточненное сообщение
